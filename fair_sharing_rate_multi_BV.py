...
for P1_noise_std_dev in [1, 1.2, 1.4]:
    for P3_noise_std_dev in [1.1, 1.3, 1.5]:
        ...
        result_dir = oj('multiplayer-BV', 'synthetic', "P1-{}_P2-{}_P3-{}".format(str(P1_noise_std_dev), str(P2_cov), str(P3_noise_std_dev)) )
        ...
        with cwd(result_dir):
            ...
            for player_index in range(N):
                ...
                np.savetxt('banzhaff_fair_{}.txt'.format(str(player_index+1)), player_banzhaff_lists[player_index])
...
for player_index in range(N):
    plt.plot(player_banzhaff_lists[player_index], label='P'+str(player_index+1))

# plt.plot(p1_banzhaff_list, linestyle='-', color='C0', label='P1')
# plt.plot(p2_banzhaff_list, linestyle='--', color='C1', label='P2')
# plt.plot(p3_banzhaff_list, linestyle='-.', color='C2', label='P3')
plt.ylabel('banzhaff value')
plt.xlabel('Iterations')
plt.legend()
plt.tight_layout()
plt.savefig('output_banzhaff_fair.pdf',  bbox_inches='tight')
# plt.show()
plt.clf()    
plt.close()
