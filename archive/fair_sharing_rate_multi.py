...
for i in range(max_iteration):
    ...
    sample_kl_3, data_x3, data_y3 = player_3.sample_kl_divergences(
        [player_sample_sizes[2]], 1, posterior_sample_size, prior_mean, prior_cov)

    player_index_data_dict = {0:[data_x1, data_y1], 1:[data_x2],\
        2:[data_x3, data_y3, player_3_noise_std_estimate]}

    sample_kls = defaultdict(float)
    # N = 3 players

    sample_kls[tuple([0])] = sample_kl_1
    sample_kls[tuple([1])] = sample_kl_2
    sample_kls[tuple([2])] = sample_kl_3
    ...
    p1_shapley_list.append(sample_shapleys[0])
    p2_shapley_list.append(sample_shapleys[1])
    p3_shapley_list.append(sample_shapleys[2])
    ...
    for j in range(len(data_x3[0][0])):
        sample_dlogL = player_3.dlogL(data_x3[0][0][j], data_y3[0][0][j], player_3_noise_std_estimate, estimated_param)
        sample_dlogL.shape = (num_params, 1)
