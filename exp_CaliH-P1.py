...
for P1_DATA_SIZE in [1000, 5000]:
    for P1_LOCAL_SAMPLE_SIZE in [10, 100, 500]:
        for P1_LOCAL_SAMPLE in ['iid' ,'lvg_iid']:

            exp_dir = oj('CaliH', "P1-{}_{}_{}".format(str(P1_DATA_SIZE), str(P1_LOCAL_SAMPLE_SIZE), str(P1_LOCAL_SAMPLE)) )
            ...
            indices_1 = np.random.choice(indices, P1_DATA_SIZE)
            ...
            indices = list(range(len(X)))
            indices_2 = np.random.choice(indices, int(P2_DATA_RATIO * len(X)))
